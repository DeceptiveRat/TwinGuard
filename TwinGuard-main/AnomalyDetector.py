...
RSSI_MIN_DRIFT = 8.0
RTT_MIN_ABNORMAL_SECONDS = 0.1
RTT_ABNORMAL_MULTIPLIER = 4.0
...
def finalize_baseline():
    ...
    if len(baseline_rssi_samples) >= 5:
        ...
        rssi_margin = max(2 * std_rssi, RSSI_MIN_DRIFT)

        baseline["RSSI_min"] = mean_rssi - rssi_margin
        baseline["RSSI_max"] = mean_rssi + rssi_margin
    ...
    if packet.get("Protocol") == "TCP":
        ...
        if baseline["RTT_mean"] is not None:
            ...
            if isinstance(rtt, (int, float)):
                ...
                rtt_threshold = max(
                    RTT_MIN_ABNORMAL_SECONDS,
                    RTT_ABNORMAL_MULTIPLIER * baseline["RTT_mean"]
                )
...
def calculate_final_score(rule_score, ml_flag):
...
def classify_final(rule_score, ml_flag):
    ...
    final_score = calculate_final_score(rule_score, ml_flag)
